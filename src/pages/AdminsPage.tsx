...
const ADMINS_COLUMNS: ExportColumn<any>[] = [
  { key: 'name', label: 'Nome' },
...
const AdminsPage: React.FC = () => {
  ...
  const [form, setForm] = useState({ name: '', sector: '', function: '', email: '', phone: '', leaderName: '', status: 'Ativo', firstLogin: true, password: '' });
  ...
  const openNew = () => { setForm({ name: '', sector: '', function: '', email: '', phone: '', leaderName: '', status: 'Ativo', firstLogin: true, password: '' }); setEditingId(null); setDialog(true); };
  const openEdit = (a: any) => {
    setForm({ name: a.name, sector: a.sector || '', function: a.function || '', email: a.email, phone: a.phone || '', leaderName: a.leader_name || '', status: a.status, firstLogin: a.first_login, password: '' });
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: (userId) => {
      const d = admins.find((a: any) => a.user_id === userId);
      const info = d ? `${d.name} (${d.email}${d.sector ? ' • ' + d.sector : ''}${d.function ? ' • ' + d.function : ''})` : userId;
      logAction('DELETE', 'profiles', userId, `Administrador excluído do sistema: ${info}. O acesso foi removido e a conta encerrada.`);
      queryClient.invalidateQueries({ queryKey: ['admin-profiles'] });
      toast({ title: '🗑️ Administrador excluído!' });
    },
    ...
    mutationFn: async () => {
      if (editingId) {
        ...
        const { data, error } = await supabase.functions.invoke('create-user', {
          body: { email: emailTrim, password: form.password, name: form.name, role: 'admin', sector: form.sector, function: form.function, phone: form.phone, leaderName: form.leaderName },
        });
        if (error) {
        ...
        if (data?.error) throw new Error(data.error);
        return { action: 'CREATE' as const, id: data?.user?.id || '' };
      }
    },
    onSuccess: (result) => {
      const detalhes = `${form.name} • E-mail: ${form.email}${form.sector ? ' • Setor: ' + form.sector : ''}${form.function ? ' • Função: ' + form.function : ''}${form.phone ? ' • Telefone: ' + form.phone : ''} • Status: ${form.status}`;
      logAction(result.action, 'profiles', result.id, `${result.action === 'CREATE' ? 'Novo administrador cadastrado' : 'Dados do administrador atualizados'}: ${detalhes}.`);
      queryClient.invalidateQueries({ queryKey: ['admin-profiles'] });
      setDialog(false);
      toast({ title: editingId ? '✅ Atualizado!' : '✅ Administrador criado!' });
    },
  ...
  const { rows: filteredAdmins, toolbarProps } = useListToolbar(admins as any[], ADMINS_COLUMNS);

  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <p className="text-sm text-muted-foreground">{filteredAdmins.length} de {admins.length} registros</p>
        <Button onClick={openNew} className="gradient-primary text-primary-foreground font-semibold"><Plus size={16} className="mr-1" /> Novo Administrador</Button>
      </div>
      <ListToolbar title="Administradores" {...toolbarProps} />
          ...
          {filteredAdmins.map((a: any) => (
            <div key={a.id} className="bg-card border border-border rounded-xl p-4 card-hover flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-muted-foreground">Líder: {a.leader_name} • {new Date(a.created_at).toLocaleDateString('pt-BR')}</p>
              </div>
      ...
      <Dialog open={dialog} onOpenChange={setDialog}>
        <DialogContent>
          <DialogHeader><DialogTitle>{editingId ? 'Editar' : 'Novo'} Administrador</DialogTitle></DialogHeader>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {deleteConfirm.reason || `Tem certeza que deseja excluir o administrador "${deleteConfirm.name}"? Esta ação não pode ser desfeita.`}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
